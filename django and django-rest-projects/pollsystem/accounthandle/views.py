...
def login_user(request):
    ...
    return render(request, 'accounthandle/login.html')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if check1 or check2 or check3:
                ...
                return redirect('accounthandle:register')
            else:
                user = User.objects.create_user(
                    username=username, password=password1, email=email)
                messages.success(
                    request, f'Thanks for registering {user.username}.', extra_tags='alert alert-success alert-dismissible fade show')
                return redirect('accounthandle:login')
    else:
        form = UserRegistrationForm()
    return render(request, 'accounthandle/register.html', {'form': form})
